...
parser.add_argument('-p', '--ionPositive', default='NA', help='Tipo de cátion (ex: Li+, Na+)')
parser.add_argument('-n', '--ionNegative', default='CL', help='Tipo de ânion (ex: F-, Cl-)')
...
modeller.addMembrane(
    ...
    positiveIon=args.ionPositive,
    negativeIon=args.ionNegative,
...
| Íon Postivo    | `{args.ionPositive}  ` |
| Íon Negativo   | `{args.ionNegative}  ` |
...
A proteína foi centralizada e inserida em uma bicamada lipídica do tipo `{args.lipid}` com uma margem de solvatação de `{args.minimumPadding} nm`, e a solução foi neutralizada com íons `{args.ionPositive}, {args.ionNegative}` para uma força iônica de `{args.ionicStrength} M`.
